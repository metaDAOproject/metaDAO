...
export default function suite() {
  ...
  describe("#initialize_proposal", async function () {
    it.only("initializes proposals", async function () {
      ...
      console.log((await this.banksClient.getClock()).slot)
      console.log("SLOT", slot);
      let lookupTableInst;
      [lookupTableInst, lookupTableAddress] = AddressLookupTableProgram.createLookupTable({
        authority: payer.publicKey,
        payer: payer.publicKey,
        recentSlot: await this.banksClient.getSlot() - 1n, // Must be a recent slot
      });
      ...
      lutTx.recentBlockhash = (await context.banksClient.getLatestBlockhash())[0];
      lutTx.feePayer = payer.publicKey;
      lutTx.sign(payer);

      await advanceBySlots(context, 1n);

      // 4. Process the transaction
      await context.banksClient.processTransaction(lutTx);

      let lutTx2 = new Transaction()
        .add(extendInstruction)
        // .add(AddressLookupTableProgram.freezeLookupTable({
        //   lookupTable: lookupTableAddress,
        //   authority: payer.publicKey,
        // }));

      lutTx2.recentBlockhash = (await context.banksClient.getLatestBlockhash())[0];
      lutTx2.feePayer = payer.publicKey;
      lutTx2.sign(payer);

      await context.banksClient.processTransaction(lutTx2);

      await advanceBySlots(context, 10000n);

      let lutTx3 = new Transaction()
        .add(extendInstruction2)

      lutTx3.recentBlockhash = (await context.banksClient.getLatestBlockhash())[0];
      lutTx3.feePayer = payer.publicKey;
      lutTx3.sign(payer);

      await context.banksClient.processTransaction(lutTx3);

      const lookupTableAccountRaw = await context.banksClient.getAccount(lookupTableAddress);

      await advanceBySlots(context, 10000n);

      const lookupTableAccount = new AddressLookupTableAccount({
        key: lookupTableAddress,
        state: AddressLookupTableAccount.deserialize(lookupTableAccountRaw.data),
      });

      console.log(lookupTableAccount);

      await advanceBySlots(context, 100n);
      ...
      const dummyTx = new VersionedTransaction(
        new TransactionMessage({
          ...
          instructions: [SystemProgram.transfer({ fromPubkey: payer.publicKey, toPubkey: dao, lamports: 10 })],
      ...
      await context.banksClient.processTransaction(dummyTx);

      console.log("DUMMY TX", dummyTx.serialize().length);
